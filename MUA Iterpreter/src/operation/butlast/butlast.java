...
public class butlast {
    public dataObject butlast(environment variableEnvironment, dataObject value, boolean isFunc, variable closureVariable)throws Exception {
        ...
        if (value.isList()) {
            ...
            for(dataObject item : value.getList()) {
                if (value.getList().indexOf(item) != value.getList().size()-1) {
            ...
            return new dataObject(value.getValue().substring(0, value.getValue().length()-1));
        }
